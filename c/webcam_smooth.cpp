...
int main(int argc, char *argv[])
{
	...
	//Capture* capture1 = cvCaptureFromCAM(-1);
	IplImage* src = NULL;
	IplImage* srcwc = NULL;
	...
	IplImage* h_plane = cvCreateImage(cvGetSize(frame), 8, 1);//we can use cvSplit()to copy each channel separately 
	...
	IplImage* hist_img = cvCreateImage(cvSize(h_bins * scale, s_bins * scale), 8, 3);   //parameter 1: width and height
